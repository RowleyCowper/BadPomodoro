...
const int POMOTIME = 20; // time for the counter in minutes

//set the initial colour - multicolour LED intensity
int colourIntensity[3] = {255,255,255};
int ledTime = 0;        // counter to control the LED dimmer
int pomoDelayTime = 0;  // variable to set the actual wait time for the counter
bool goingUp = true;    // probably not really needed any more...but left over from when I had it oscillate up and down
// flag for starting the timer
bool buttonPush = false;

void setup()  
{
  pomoDelayTime = 60000 * POMOTIME / 255;
...
void loop()
{
  ...
  while (buttonPush == false) 
  {
    // read in the button states
    buttonStates[BUTTON_WHITE] = 1 - digitalRead(BUTTON_WHITE);
    if (buttonStates[BUTTON_WHITE]==1)  // if the button is pushed
    { 
      buttonPush = true; // change the logic flag and start the timer
    }
  }

  // write the LED colour
  analogWrite(LED_WHITE_PIN,ledTime);
  ...
  // Serial.print(colourIntensity[0]);
  // Serial.print(colourIntensity[1]);
  // Serial.println(colourIntensity[2]);
  ...
  //   }
  // wait for 0.01 of a second (1000 = 1 second).  POMOTIME is in minutes.  60000 ticks is 1 minute.
  delay(pomoDelayTime);
}
...
void winCelebration()
{
  // const int songLength = 19;  // star wars sets the number of notes of the song
  // const int songLength = 43; // ABC song
  const int songLength = 4; // ABC song
